...
class TpaHereCommand extends BaseCommand {
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        ElysiumUtils::$tpahereRequests[$target->getName()] = [
            'requester' => $sender->getName(),
            'time' => time()
        ];

        $target->sendMessage(C::colorize("&r&l&a(!) &r&aYou have received a teleport request from " . $sender->getNameTag() . " &r&7(/tpaccept)"));
        ...
        Loader::getInstance()->getScheduler()->scheduleDelayedTask(new class($target->getName()) extends Task {
            ...
            public function onRun(): void {
                ElysiumUtils::checkRequestTimeout($this->targetName, 'tpahereRequests');
            }
        }, 12000);
